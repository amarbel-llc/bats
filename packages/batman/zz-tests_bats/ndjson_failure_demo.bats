...
# NDJSON files exercise both `ok: true` and `ok: false` record shapes
# (per amarbel-llc/tap RFC 0001). Under the default `splitNdjson = true`,
# the failing case lands in `run.failures.ndjson` and the passing case
# lands in `run.passes.ndjson`.
#
